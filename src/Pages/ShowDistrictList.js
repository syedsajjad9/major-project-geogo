...
const ShowDistrictList = (props) => {
    ...
    return (
        <div className="DataShowing">
            <h2 style={{textAlign:'center',marginBottom:'15px', color:'#2B2B52',marginTop:'25px',backgroundColor:'white',padding:'5px',margin:'5px',}}>{`Showing Result for Age: ${props.ageGroup} +`}</h2>
            {
                mydata.length>=0 ?
                mydata.map(d=>{
                    return(
             ...
             {
              isAvailable ? speak("vaccine is available, please scroll down, you'll find green") : speak("sorry vaccine is not available, stay here, we will notify you once the vaccine is available")
            }
        </div>
